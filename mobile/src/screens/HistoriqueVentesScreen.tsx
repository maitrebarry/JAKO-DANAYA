...
import { cancelLivraison, cancelPaiementClient, fetchHistoriqueVentesBoutique, type HistoriqueVenteItemDTO } from '../services/historique';
import { downloadAndSharePdf } from '../services/pdf';
import { hasPermission, isSuperAdmin } from '../utils/permissions';
import { showError, showSuccess } from '../utils/notify';
import { useFormatMoney } from '../utils/currency';

type FilterType = 'ALL' | 'LIVRAISON' | 'PAIEMENT';
...
export default function HistoriqueVentesScreen() {
  ...
  const canAnnulerLivraison = isSuperAdmin(profile) || hasPermission(profile, ['RECEPTION_ANNULATION', 'RECEPTION_SUPPRESSION']);
  const canSeeCaisse = isSuperAdmin(profile) || hasPermission(profile, ['CAISSE_VOIR', 'CAISSE_LECTURE']);

  const [loading, setLoading] = useState(true);
  const [items, setItems] = useState<HistoriqueVenteItemDTO[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const list = await fetchHistoriqueVentesBoutique(boutiqueId, token, { annulations: viewingAnnulations });
      setItems(list);
    } catch (e: any) {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      list = list.filter((i) => {
        const s = `${i.reference || ''} ${i.referenceCommande || ''} ${i.client || ''}`.toLowerCase();
        return s.includes(q);
      });
    }
    return list;
  ...
  const printLivraisonPdf = async (id: number) => {
    if (!token) return;
    try {
      await downloadAndSharePdf({ apiPath: `livraisons/${id}/pdf`, token, filename: `livraison-${id}.pdf` });
    } catch (e: any) {
  ...
  const printPaiementPdf = async (id: number) => {
    if (!token) return;
    try {
      await downloadAndSharePdf({ apiPath: `paiements-clients/${id}/pdf`, token, filename: `paiement-client-${id}.pdf` });
    } catch (e: any) {
      showError('Erreur', e?.message || "Impossible d'ouvrir le PDF");
    }
  };

  const annulerItem = (item: HistoriqueVenteItemDTO) => {
    const isPaiement = item.type === 'PAIEMENT';
    if (isPaiement && !canAnnulerPaiement) {
      showError('Accès refusé', 'Permission manquante');
      return;
    }
    if (!isPaiement && !canAnnulerLivraison) {
      showError('Accès refusé', 'Permission manquante');
      return;
    }
    Alert.alert('Annuler', `Annuler ${isPaiement ? 'ce paiement' : 'cette livraison'} ?`, [
      { text: 'Retour', style: 'cancel' },
      {
        ...
        onPress: async () => {
          if (!token) return;
          try {
            if (isPaiement) await cancelPaiementClient(item.id, 'Annulation via historique', token);
            else await cancelLivraison(item.id, 'Annulation via historique', token);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      <View style={{ padding: 16, paddingBottom: 8 }}>
        <Text style={{ color: theme.text, fontSize: 20, fontWeight: '900', marginBottom: 10 }}>Historique ventes</Text>

        <View style={{ flexDirection: 'row', gap: 8 as any, marginBottom: 10 }}>
          {(['ALL', 'LIVRAISON', 'PAIEMENT'] as FilterType[]).map((t) => (
            <Pressable
              key={t}
            ...
            >
              <Text style={{ color: theme.text }}>{t === 'ALL' ? 'Tout' : t === 'LIVRAISON' ? 'Livraisons' : 'Paiements'}</Text>
            </Pressable>
          ))}
        </View>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View style={{ backgroundColor: theme.card, borderRadius: 14, padding: 12, borderWidth: 1, borderColor, marginBottom: 10 }}>
                ...
                <Text style={{ color: theme.muted, marginTop: 2 }}>{item.client || '—'}</Text>
                {canSeeCaisse && item.referenceCaisse ? <Text style={{ color: theme.muted, marginTop: 2 }}>Caisse: {item.referenceCaisse}</Text> : null}
                ...
                <View style={{ flexDirection: 'row', gap: 10 as any, marginTop: 12 }}>
                  <Pressable
                    onPress={() => (isPaiement ? printPaiementPdf(item.id) : printLivraisonPdf(item.id))}
